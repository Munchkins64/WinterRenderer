...
void VulkanRenderer::run(const std::string& modelPath) {
    initWindow();
    initVulkan(modelPath);
    mainLoop();
    cleanup();
}
...
void VulkanRenderer::initWindow() {
    ...
    window = glfwCreateWindow(width, height, "Dodo - damn", nullptr, nullptr);
    ...
    setWindowIcon(window, "model/dodo-logo.png");
}

void VulkanRenderer::initVulkan(const std::string& modelPath) {
    createInstance();
    setupDebugMessenger();
    createSurface();
    pickPhysicalDevice();
    ...
    if (!model->loadFromFile(modelPath, device, physicalDevice, commandPool, graphicsQueue)) {
        throw std::runtime_error("Failed to load model!");
    }

    // Position camera based on model bounds
    glm::vec3 center = (model->minBounds + model->maxBounds) * 0.5f;
    float size = glm::length(model->maxBounds - model->minBounds);
    camera.position = center + glm::vec3(0, size * 0.5f, size * 1.5f);

    audioManager = std::make_unique<AudioManager>();
    if (audioManager->init()) {
        // Play background music (optional)
        audioManager->playMusic("model/Aria Math.mp3", 0.5f);
    }
...
void VulkanRenderer::processInput() {
    ...
    // Toggle cursor lock with TAB key - ADD THIS
    ...
    // Only process movement when cursor is locked - UPDATE THIS
    if (cursorLocked) {
...
void VulkanRenderer::createInstance() {
    ...
    appInfo.pApplicationName = "Dodo - version 0.1";
...
void VulkanRenderer::createDescriptorSets() {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        imageInfo.imageView = model->textures[0].imageView;
        imageInfo.sampler = model->textures[0].sampler;
...
void VulkanRenderer::updateUniformBuffer(uint32_t currentImage) {
    ...
    // Start with centering the model
    ubo.model = glm::translate(glm::mat4(1.0f), -center);

    // Fix coordinate system: rotate -90 degrees around X-axis (Z-up to Y-up)
    ubo.model = glm::rotate(glm::mat4(1.0f), glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f)) * ubo.model;

    // Apply user rotation (Q/E keys)
    ubo.model = glm::rotate(glm::mat4(1.0f), glm::radians(modelRotation), glm::vec3(0.0f, 1.0f, 0.0f)) * ubo.model;
    ...
    ubo.lightPos = camera.position + glm::vec3(2.0f, 2.0f, 0.0f);
...
void VulkanRenderer::mouseCallback(GLFWwindow* window, double xposIn, double yposIn) {
    ...
    // Only move camera when cursor is locked - ADD THIS CHECK
    if (!app->cursorLocked) {
        return;
    }
...
void VulkanRenderer::scrollCallback(GLFWwindow* window, double xoffset, double yoffset) {
    ...
    // Only zoom when cursor is locked - ADD THIS CHECK
    if (!app->cursorLocked) {
        return;
    }
